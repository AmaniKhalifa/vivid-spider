...
from imdb.items import TVshow
from scrapy.http import Request
import re
from urlparse import urljoin

class imdbTVSeriesSpider(CrawlSpider):
    name = "imdb_tvseries"
    ...
    start_urls = ["http://www.imdb.com/search/title?at=0&num_votes=5000,&sort=user_rating,desc&start=1&title_type=tv_series"]
    ...
    def get_images_videos(self,response):
        ...
        if images_url != None :
            ...
            yield Request(movie_url,meta={'videos':'', 'images':''},dont_filter=True,callback=self.parse_tv_series)
    ...
    def get_all_images(self,response):
        ...
        if videos_url != None:
            ...
            yield Request(movie_url,meta = {'images':images,'videos':''},dont_filter=True,callback=self.parse_tv_series)
    ...
    def get_all_videos(self,response):
        ...
        movie_url = response.request.meta['movie_url']
        yield Request(movie_url,meta={'images':images,'videos':videos},callback=self.parse_tv_series,dont_filter=True)
    ...
    def parse_tv_series(self, response):
        sel = Selector(response)
        item = TVshow()
        ...
        item['show_name'] = names[0] if len(names) > 0 else ''
        ...
        item['ep_duration'] = dur[0] if len(dur) > 0 else ''
        ...
        release = sel.xpath('//td[@id="overview-top"]//span[@class="nobr"]/text()').extract()
        release = re.findall(r'\d+',release[0]) if len(release) > 0 else []
        item['start_date'] = release[0] if len(release) > 0 else ''
        item['end_date'] = release[1] if len(release) > 1 else '' 
        ...
        seasons_numbers = sel.xpath('//div[@id="titleTVSeries"]/div[1]//a/text()').extract()
        seasons_links = sel.xpath('//div[@id="titleTVSeries"]/div[1]//a/@href').extract()

        item['seasons'] = self.gen_double_list("number",seasons_numbers,"link",map(lambda a: urljoin(response.url,a),seasons_links) )
        return item
